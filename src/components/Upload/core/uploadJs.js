...
class JiuLawUpload {
  constructor (op) {
    ...
    this.serverName_ = op.serverName && op.serverName != '' ? op.serverName : 'ept-collaboration';
   ...
   * @param uploadType
   * @param params
   * @param transParams  上传过程透传的参数,前端(同时会在失败／成功后返回)／上传服务端都会收到这个透传参数
   * @param callback
   * @param callProgress
   */
  upload (uploadType, params, transParams, callback, callProgress) {
    switch (this.platform_) {
    case constant.plat_aliyun:
      // paserParams
      this.upload_aliyun(uploadType, params, transParams, callback, callProgress);
      break;
    default:// 阿里云
   ...
   * @param callback
   */
  upload_aliyun (uploadType, params, transParams, callback, progressCall) {
    ...
    if (transParams && Object.keys(transParams)) {
      megreJson(transParams, data);
    }
          ...
          for (var instanceIdx = 0; instanceIdx < this.uploadFileQuene.length; instanceIdx++) {
            ...
            if (reqInstance.key == key) {
              ...
              if (this.waitFielQuene.length > 0) {
                console.log('waitFielQuene', key, reqInstance.key, instanceIdx, JSON.parse(JSON.stringify(this.waitFielQuene)));
                let waitInstance = this.waitFielQuene[0];
                this.upload_aliyun(
                  waitInstance.uploadType,
                  waitInstance.params,
                  waitInstance.transParams,
